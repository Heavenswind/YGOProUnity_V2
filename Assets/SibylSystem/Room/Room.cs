...
public class Room : WindowServantSP
{
    ...
    public void StocMessage_RoomList(BinaryReader r)
    {
        ...
        {
            uint RoomID = r.ReadUInt32(); // Room ID
            byte Mode = r.ReadByte(); // 0 = single, 1 = match, 2 = tag
            byte Region = r.ReadByte(); // 0 = tcg, 1 = ocg, 2 = all
            bool NoBanlist = r.ReadBoolean(); // No banlist?
            GameState State = (GameState)r.ReadByte(); // Game State Enum
            byte DuelCount = r.ReadByte();
            byte TurnCount = r.ReadByte();
            string Team1Name = r.ReadUnicode(20).Trim();
            byte Team1Score = r.ReadByte(); // Win count
            int Team1LP = r.ReadInt32();
            string Team2Name = r.ReadUnicode(20).Trim();
            byte Team2Score = r.ReadByte(); // Win count
            int Team2LP = r.ReadInt32();

            string hoststr = string.Empty;
            StringBuilder roomTag = new StringBuilder();
            if (Region == 1)
                roomTag.Append("[11C69C][TCG]");
            else if (Region == 2)
                roomTag.Append("[11C69C][OCG]");
            if (Mode == 0)
                roomTag.Append("[8AE57E][Duel]");
            else if (Mode == 1)
                roomTag.Append("[8AE57E][Match]");
            else if (Mode == 2)
                roomTag.Append("[8AE57E][Tag]");
            if (NoBanlist)
                roomTag.Append("[C63111][No Banlist]");
    ...
    public void StocMessage_PlayerInfo(BinaryReader r)
    {
        if (r.ReadBoolean()) // success ?
        {
            string nickname = r.ReadUnicode(20);
            ushort rating = r.ReadUInt16();
        }
        AddChatMsg(r.ReadALLUnicode(), 8);

        Program.I().shiftToServant(Program.I().roomList);
    }
