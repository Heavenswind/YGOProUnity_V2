...
public static class TcpHelper
{
    ...
    public static bool roomListChecking = false;
    ...
    public static void Join(string RoomID)
    {
        if (canjoin)
        {
            if (tcpClient == null || tcpClient.Connected == false)
            {
                ...
                try
                {
                    tcpClient = new TcpClientWithTimeout(SelectServer.ServerIP, int.Parse(SelectServer.ServerPort), 3000).Connect();
    ...
    public static void Create(Package msg)
    {
        if (canjoin)
        {
            if (tcpClient == null || tcpClient.Connected == false)
        ...
        if (canjoin)
        {
            if (tcpClient == null || tcpClient.Connected == false)
            {
                ...
                try
                {
                    ...
                    Thread t = new Thread(Receiver);
                    t.Start();
                    CtosMessage_PlayerInfo(username, password);
                }
                catch (Exception e)
                {
                    Program.DEBUGLOG("onDisConnected 10");
                    ...
                    try
                    {
                        ...
                        switch (ms)
                        {
                            case StocMessage.GameMsg:
                                ...
                                break;
                            case StocMessage.PlayerInfo:
                                Program.I().room.StocMessage_PlayerInfo(r);
                                break;
                            ...
                            case StocMessage.RoomList:
                                Program.I().room.StocMessage_RoomList(r);
                                break;
                            default:
                                break;
                        }
    ...
    public static void CtosMessage_RoomList()
    {
        Package message = new Package
        {
            Fuction = (int)CtosMessage.RoomList
        };
        Send(message);
    }
    ...
    public static void CtosMessage_PlayerInfo(string username, string password)
    {
        Package message = new Package();
        message.Fuction = (int)CtosMessage.PlayerInfo;
        message.Data.writer.Write((Int16)Config.ClientVersion);
        message.Data.writer.WriteUnicode(Encrypt(username+password), 33);
        Send(message);
    }

    public static void CtosMessage_CreateGame(Package msg)
    {
        deckStrings.Clear();       
        Send(msg);
    }
    ...
    public static void CtosMessage_JoinGame()
    {
        deckStrings.Clear();
        Package message = new Package();
